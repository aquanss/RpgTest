...
// FIX: Changed import from non-existent 'Rarity' to the correct 'RarityKey' type.
...
// FIX: Updated getItemScore to use RarityKey enum values for logic instead of hardcoded Turkish strings.
            ...
            <div className={gridClasses}>
                {filteredInventory.map((item, index) => {
                    ...
                    return (
                        ...
                        >
                            <img src={item.icon} alt={translatedName} className="w-12 h-12 object-contain pointer-events-none" loading="lazy" decoding="async" />
                            {item.quantity > 1 && (
                            ...
                            <div className="absolute -top-2 left-1/2 -translate-x-1/2 -translate-y-full w-48 bg-[var(--background-dark)] text-[var(--text-primary)] text-sm rounded-lg p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none shadow-lg z-20 border border-[var(--border-color)]">
                                ...
                                {/* FIX: Cast to 'any' because UITranslationKey type is not available for import. */}
